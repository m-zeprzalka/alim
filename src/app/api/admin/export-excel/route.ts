...
export async function GET(request: NextRequest) {
  try {
    ...
    formsSheet.columns = [
      ...
      {
        header: "Szczeg.podstawy ustaleń",
    ...
    financeSheet.columns = [
      ...
      {
        header: "Renta rodzinna - kwota",
        key: "rentaRodzinnaKwota",
        width: 15,
      },
      {
      ...
      {
        header: "Świadczenie pielęgnacyjne - kwota",
    ...
    formSubmissions.forEach((submission) => {
      ...
      subscriptionsSheet.addRow({
        ...
        createdat: submission.emailSubscription.createdAt
          ? new Date(submission.emailSubscription.createdAt).toLocaleString(
              "pl-PL"
            )
          : null,
        updatedat: submission.emailSubscription.updatedAt
          ? new Date(submission.emailSubscription.updatedAt).toLocaleString(
              "pl-PL"
            )
          : null,
        subscriptionsubmittedat: submission.emailSubscription.submittedAt
          ? new Date(submission.emailSubscription.submittedAt).toLocaleString(
              "pl-PL"
            )
          : null,
        verifiedat: submission.emailSubscription.verifiedAt
          ? new Date(submission.emailSubscription.verifiedAt).toLocaleString(
              "pl-PL"
            )
          : null,
        verificationtoken: submission.emailSubscription.verificationToken,
        acceptedterms: submission.emailSubscription.acceptedTerms
          ? "Tak"
          : "Nie",
        acceptedcontact: submission.emailSubscription.acceptedContact
          ? "Tak"
          : "Nie",
      });
        ...
        if (formData.kosztyDzieci && Array.isArray(formData.kosztyDzieci)) {
          formData.kosztyDzieci.forEach((koszt: Record<string, any>) => {
            financeSheet.addRow({
              ...
              brakDodatkowychZrodel: koszt.inneZrodlaUtrzymania
