...
export default function KosztyUtrzymania() {
  ...
  const handleNext = () => {
    ...
    if (aktualneDzieckoIndex < kosztyDzieci.length - 1) {
      // Przejście do następnego dziecka
  ...
  const handleBack = () => {
    if (aktualneDzieckoIndex > 0) {
      // Cofamy się do poprzedniego dziecka
      setAktualneDzieckoIndex(aktualneDzieckoIndex - 1);
    } else {
      // Wracamy do poprzedniego kroku
      // Sprawdzamy, czy były dzieci z modelem "inny" - jeśli tak, wracamy do czasu opieki
      const dzieckoInny = formData.dzieci?.find(
